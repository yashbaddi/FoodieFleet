...
- **Backend (`apps/node-server`)**: Node.js (ESM), Express 4, `express-ws` (WebSockets), `pg` (PostgreSQL client), `ioredis` / `redis` (Redis client & Geospatial queries), `swagger-jsdoc` & `swagger-ui-express` (OpenAPI 3.0), `bcryptjs`, `jsonwebtoken`
...
└── apps/
    ...
    │   ├── swagger.js            # Swagger JSDoc / OpenAPI 3.0 specification setup
    ...
    │   │   ├── auth.js           # /api/auth (login, register, logout, me) + Swagger docs
...
When modifying or adding backend functionality in `apps/node-server`, strictly adhere to this separation:
- **`routes/`**: Define HTTP methods, paths, middleware (`authMiddleware`, `bodyParser`), and OpenAPI Swagger JSDoc annotations. Delegate handling immediately to a controller.
...
- Every route in `apps/node-server/routes/*.js` must include JSDoc `@openapi` comments describing the path, HTTP method, parameters, request bodies, tags, and response codes.
...
- **DO** update `@openapi` JSDoc annotations when modifying or creating routes.
...
4. Add route with `@openapi` documentation in `apps/node-server/routes/<entity>.js`.
